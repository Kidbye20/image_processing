...
cv::Mat non_local_means_color(const std::vector<cv::Mat>& noise_channels, const int search_radius=5, const int radius=2, const int sigma=1, const char* kernel_type="mean") {
    ...
    const double sigma_2_inv = 1. / (sigma * sigma);
    // 收集目标图像的信息
    cv::Mat denoised_B = noise_channels[0].clone();
    cv::Mat denoised_G = noise_channels[1].clone();
    cv::Mat denoised_R = noise_channels[2].clone();
    const int H = denoised_B.rows;
    const int W = denoised_B.cols;
    // 将图像 padding 一下
    const auto padded_B = make_pad(denoised_B, radius, radius);
    const auto padded_G = make_pad(denoised_G, radius, radius);
    const auto padded_R = make_pad(denoised_R, radius, radius);
    const int H2 = padded_B.rows;
    const int W2 = padded_B.cols;
    const uchar* const noise_B_ptr = noise_channels[0].data;
    const uchar* const noise_G_ptr = noise_channels[1].data;
    const uchar* const noise_R_ptr = noise_channels[2].data;
    const uchar* const padded_B_ptr = padded_B.data;
    const uchar* const padded_G_ptr = padded_G.data;
    const uchar* const padded_R_ptr = padded_R.data;
    // 现在开始滤波, 求目标图像中的每一点
    for(int i = 0;i < H; ++i) {
        const int up = std::max(radius, i - search_radius);
        const int down = std::min(H2, i + search_radius);
        for(int j = 0;j < W; ++j) {
            // 当前要去噪的点 (i, j), 以它为中心的区域的点, 我得收集起来
            uchar source_B[window_size];
            for(int t = 0;t < window_len; ++t)
                std::memcpy(source_B + t * window_len, padded_B_ptr + (i + t) * W2 + j, window_len * sizeof(uchar));
            uchar source_G[window_size];
            for(int t = 0;t < window_len; ++t)
                std::memcpy(source_G + t * window_len, padded_G_ptr + (i + t) * W2 + j, window_len * sizeof(uchar));
            uchar source_R[window_size];
            for(int t = 0;t < window_len; ++t)
                std::memcpy(source_R + t * window_len, padded_R_ptr + (i + t) * W2 + j, window_len * sizeof(uchar));
            // 累计值 和 权重总和
            double sum_value_B = 0, sum_value_G, sum_value_R;
            double weight_sum = 0;
            double weight_max = -1e3;
            // 每个点先确认它目前的搜索区域有多大, 为什么是 radius?
            const int left = std::max(radius, j - search_radius);
            ...
            for(int x = up; x < down; ++x) {
                for(int y = left; y < right; ++y) {
                    ...
                    // 当前对比的区域是以 x, y 为中心, 半径为 radius 的区域
                    // 我得把这个区域的值都找出来, 收集起来
                    uchar target_B[window_size];
                    for(int t = 0;t < window_len; ++t)
                        std::memcpy(target_B + t * window_len, padded_B_ptr + (x - radius + t) * W2 + y - radius, window_len * sizeof(uchar));
                    uchar target_G[window_size];
                    for(int t = 0;t < window_len; ++t)
                        std::memcpy(target_G + t * window_len, padded_G_ptr + (x - radius + t) * W2 + y - radius, window_len * sizeof(uchar));
                    uchar target_R[window_size];
                    for(int t = 0;t < window_len; ++t)
                        std::memcpy(target_R + t * window_len, padded_R_ptr + (x - radius + t) * W2 + y - radius, window_len * sizeof(uchar));
                    // 然后计算两个区域的相似度
                    double distance = 0.0;
                    for(int k = 0;k < window_size; ++k) {
                        double res_B = static_cast<double>(target_B[k] - source_B[k]);
                        double res_G = static_cast<double>(target_G[k] - source_G[k]);
                        double res_R = static_cast<double>(target_R[k] - source_R[k]);
                        distance += weights_kernel[k] * (res_B * res_B + res_G * res_G + res_R * res_R) / 3;
                    }
                    const double cur_weight = std::exp(-distance * sigma_2_inv);
                    // 记录当前最大的权值
                    if(cur_weight > weight_max) weight_max = cur_weight;
                    // 累加值
                    sum_value_B += cur_weight * padded_B.at<uchar>(x, y);
                    sum_value_G += cur_weight * padded_G.at<uchar>(x, y);
                    sum_value_R += cur_weight * padded_R.at<uchar>(x, y);
                    weight_sum += cur_weight;
                }
            }
            // 搜索结束
            sum_value_B += weight_max * noise_channels[0].at<uchar>(i, j);
            sum_value_G += weight_max * noise_channels[1].at<uchar>(i, j);
            sum_value_R += weight_max * noise_channels[2].at<uchar>(i, j);
            weight_sum += weight_max;
            denoised_B.at<uchar>(i, j) = cv::saturate_cast<uchar>(sum_value_B / weight_sum);
            denoised_G.at<uchar>(i, j) = cv::saturate_cast<uchar>(sum_value_G / weight_sum);
            denoised_R.at<uchar>(i, j) = cv::saturate_cast<uchar>(sum_value_R / weight_sum);
        }
    }
    cv::Mat denoised;
    cv::merge(std::vector<cv::Mat>({denoised_B, denoised_G, denoised_R}), denoised);
    return denoised;
}
...
cv::Mat non_local_means(const cv::Mat& noise_image, const int search_radius, const int radius, const int sigma, const char* kernel_type, const bool fast, const bool multi_channel) {
    ...
    // 彩色图的 non_local_means
        ...
        if(!multi_channel) {
            ...
            return non_local_means_color(bgr_channels, search_radius, radius, sigma, kernel_type);
    }
    return noise_image;
}
