...
def exposure_fusion(sequence, alphas=(1.0, 1.0, 1.0), best_illumination=0.5, sigma=0.2, eps=1e-12, 
    ...
    weights += eps
    weights_sum = numpy.sum(weights, axis=0)
    weights /= numpy.expand_dims(weights_sum, axis=0)
    # 这里要把 sequence 还原回来
    sequence *= 255;
    # 粗糙的融合
    ...
    if(use_gaussi == True):
        # 使用高斯模糊对 weights 做模糊
        smoothed_weights = numpy.stack([cv2.GaussianBlur(w, (7, 7), 1.0) for w in weights], axis=0)
    ...
    if(use_lappyr == True):
        ...
        def build_laplace_pyramaid(gaussi_pyramid, layers):
            pyramid = [gaussi_pyramid[layers - 1]] # 最低分辨率也放到这里来, 不是细节, 但后面不用单独融合最底层的起始图像, 方便
...
# 曝光融合
fused_results = exposure_fusion(sequence, use_lappyr=True, use_gaussi=True, layers_num=7)
